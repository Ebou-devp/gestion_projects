...
try {
    ...
    // Préparer une requête SQL pour supprimer le projet de la base de données
    $stmt = $pdo->prepare("DELETE FROM projects WHERE id = :id");
    $stmt->execute(['id' => (int)$_GET['id']]); // Exécuter la requête avec l'ID du projet passé en paramètre
    echo "Projet supprimé définitivement.<br>"; // Afficher un message de confirmation (utile pour le débogage)
